package com.nmg.multithread.item;
...
public class ItemAsyncService  {
	...
	public ItemAsyncService(Context pimContext) {
	...
	public void getOffers(ArrayList<String> offers, Item item )  {	
 ...
 
		{
			...
			for (String offer : offers) {
				 ...
				 Callable<HashMap<String, String>> callable = new FetchItem(pimContext, offer , "item iLiNK Offer Hierarchy");
	...
	public  void getOtherOffers(String primaryKey, Item item){
		...
		try {
				...
				setOtherItemIDArray("iLiNK Item Style Spec/Identifiers/Offers", item); 
			
			    getOffers(offers , item  );
	...
	try {
		...
		// This will list an array of offer items	 
		...
		for (AttributeInstance innerInstance : innerInstances) {
			...
			for (AttributeInstance innerInstance12 : innerInstances1) {
				...
				if ( innerInstance12.getPath().endsWith("Offer Item iLiNK Id") ) {
				
                  offers.add( innerInstance12.getDisplayValue()) ;
